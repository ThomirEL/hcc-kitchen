    ...
    public void RenderAllCansNow()
{
    ...
    {
        Debug.LogWarning("[CanManager] Render already in progress.");
        return;
    }
    ...
    {
        Debug.LogWarning("[CanManager] No matching cans found.");
        return;
    }
...
void Update()
    {
        // For testing: Press 'R' to render all cans again
        if (Keyboard.current.rKey.wasPressedThisFrame)
        {
            Debug.Log("[CanManager] 'R' pressed - refreshing all cans.");
            Debug.Log("_isRenderingSequence: " + _isRenderingSequence);
            RenderAllCansNow();
        }
    }
